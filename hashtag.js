...
function generateHashtag(str) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    arr[i] = arr[i].charAt(0).toUpperCase() + arr[i].slice(1); // to turn the first character in each element into CAPS and join the remainder of the array together
  }
  let strNew = arr.join("").replace(/\s/g, ""); // turn array into string and remove space
  if (strNew === "" || strNew.length === 0) {
    return false;
  } else if (strNew.length >= 140) {
    return false;
  } else {
    return "#" + strNew;
  }
